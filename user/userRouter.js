...
router.get("/:id", (req, res) => {
  User.findById({ _id: req.params.id })
    ...
    .catch(function(error) {
      res
        .status(500)
        .json(`{Error occured while getting a user by ID: ${error}}`);
    });
});
  ...
  User.findOne({ username }).then(user => {
    ...
    User.create(req.body)
      ...
      .catch(err => {
        res
          .status(500)
          .json({ error: "Something went wrong while creating account!" });
      });
  });
});
      ...
      if (user) {
        user
          ...
          .catch(err => {
            res
              .status(500)
              .json(`{ error: error processing information. error: ${err}}`);
          });
      } else {
        res.status(401).json({ error: "Invalid username or password!" });
      }
