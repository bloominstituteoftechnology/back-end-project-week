...
describe('POST Method to /notes with missing title', () => {
  it('should return an error message', async () => {
    const res = await req(server).post('/notes').send({
      ...
      textBody:'Content vai Jest. The best testing framework out right now!'
    })
...
describe('POST Method to /notes with missing textBody', () => {
  it('should return an error message', async () => {
    const res = await req(server).post('/notes').send({
      title:'Title via Jest',
      // textBody:'Content vai Jest. The best testing framework out right now!'
...
describe('POST Method to /notes', () => {
  it('return post object and check textBody and id properties', async () => {
    const res = await req(server).post('/notes').send({
      title:'Title via Jest',
      textBody:'Content vai Jest. The best testing framework out right now!'
    })
    ...
    expect(res.status).toEqual(statusCodePass)
    expect(res.body).toHaveProperty("textBody", "Content vai Jest. The best testing framework out right now!")
...
describe('POST Method to /notes/id with an invalid id', () => {
  it('return a message saying invalid id', async () => {
    const res = await req(server).post('/notes/29').send({
      title:'Title via Jest',
      textBody:'Content via Jest. The best testing framework out right now!'
    })
...
describe('POST Method to /notes/id with an valid id', () => {
  it('return the updated note', async () => {
    ...
    expect(res.status).toEqual(statusCodePass)
    expect(res.body).toHaveProperty("textBody", "Content via Jest. The best testing framework out right now!")
    expect(res.body).toHaveProperty("id",11)
