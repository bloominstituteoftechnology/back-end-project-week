...
class Edit extends Component {
	...
	if (note) {
	    ...
	    axios
		.get(`http://localhost:8000/api/view/`, {title: this.state.title, text: this.state.text, id: this.state.id})
		.then(response => {
	...
	axios
	    .put(`http://localhost:8000/api/edit/${this.props.match.params.id}`, myNote2)
	    .then(response => {
