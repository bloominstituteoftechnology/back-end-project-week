...
describe('Auth Router', () => {
    ...
    describe('register', () => {
        it('should block users from registering with missing username', async () => {
            ...
            expect(res.body).toEqual({ error: "username must be included and must be a string" })
          })
          it('should block users from registering with missing password', async () => {
            ...
            expect(res.statusCode).toEqual(400)
            expect(res.body).toEqual({ error: "password must be included and must be a string" })
          })
